...
var slack = function() {
  this.token = PropertiesService.getScriptProperties().getProperty('SLACK_LEGACY_TOKEN');
  this.slackApp = SlackApp.create(this.token);
  ...
  */
  this.postSlackMessage = function(mes, profile) {
    ...
    this.slackApp.postMessage(this.channels, mes, this.options);  
  }
  ...
  */
  this.postSlackFiles = function(image) {
    var options = {channels: this.channels};
